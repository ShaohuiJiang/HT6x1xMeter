 ...
 *         上电状态：先进行上电状态初始化，然后进入死循环，循环中执行各种任务，并且时不时判断外部电源
 *                  如果判断到掉电，保存重要参数后，就跳出上电循环，再次进行电源判断；
...
int main(void)
{
    ...
    while (1)       //大的死循环，除非复位，不然程序永远在里面运行
    {
        ...
        if(POWERON == Check_Power(100))          //判断外部电源状态是有电状态
        {
            /* 外部电源供电状态初始化 */
            ...
            while(1)                        //上电状态循环
            {
                ...
                if(POWEROFF == Check_Power(10))  //再次判断外部电源状态，假如是掉电，那么从上电死循环跳出来
                {
                    break;
                }
        ...
        else                                //判断到外部电源状态是无电状态
        {
            ...
            while(1)                       //下电状态循环
            {
                ...
                if(POWERON == Check_Power(10))  //判断外部电源状态，假如是上电，那么从掉电死循环跳出来
                {
                    break;
                }
                //添加各种任务1
